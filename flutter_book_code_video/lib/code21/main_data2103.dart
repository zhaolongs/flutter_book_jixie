...
///  XOY二维 坐标轴
class CanvasXYePage extends StatefulWidget {
  @override
...
class XOYPainter extends CustomPainter {
  ...
  void drawXLine(Canvas canvas, Size size) {
    ...
    ///绘制x轴向右的箭头
    canvas.drawLine(Offset(endXOffest.dx-10,endXOffest.dy-10), endXOffest, _paint);
    canvas.drawLine(Offset(endXOffest.dx-10,endXOffest.dy+10), endXOffest, _paint);
    ...
    for(int i=1;i<xNumber;i++){
      ...
      //绘制竖直的刻度
      canvas.drawLine(Offset(startX,endXOffest.dy-5), Offset(startX,endXOffest.dy+5), _paint);
    }
  ...
  void drawYLine(Canvas canvas, Size size) {
    ...
    ///绘制y轴向下的箭头
    canvas.drawLine(Offset(endYOffest.dx-10,endYOffest.dy-10), endYOffest, _paint);
    canvas.drawLine(Offset(endYOffest.dx+10,endYOffest.dy-10), endYOffest, _paint);
    ...
    for(int i=1;i<yNumber;i++){
      ...
      ///对于轴上的刻度来讲，
      canvas.drawLine(Offset(startYOffest.dx-5,startY), Offset(startYOffest.dx+5,startY), _paint);
    }
  }
...
///  XOY二维 坐标轴 绘制原点
  void drawOriginPoint(Canvas canvas,Size size){
    ...
    canvas.drawPoints(PointMode.points, [Offset(padding.left,padding.top)], _paint);
  }
}
